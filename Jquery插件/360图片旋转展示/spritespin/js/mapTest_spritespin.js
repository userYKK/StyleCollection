...
(function($, window) {
    ...
    Spin.prepareBackground = function(data){
        //console.info('prepareBackground');
...
(function($, window, Spin){
    Spin.behaviors.click = {
        name : "click",
        mouseup    : function(e){
            //console.info('behaviors.click.mouseup');
...
(function($, window, Spin){
    Spin.behaviors.drag = {
        ...
        mousedown  : function(e){
            //console.info('behaviors.drag.mousedown');
            ...
            // 点击放大效果
           /* var _index = data.frame;
            var img = data.images[_index];
            $(img).animate({
        ...
        mousemove  : function(e){
            //console.info('behaviors.drag.mousemove');
            var $this = $(this), data = $this.data('spritespin');
            if (data.onDrag){
                Spin.updateInput(e, data);
        ...
        mouseup    : function(e){
            //console.info('behaviors.drag.mouseup');

            var $this = $(this), data = $this.data('spritespin');
            Spin.resetInput(data);
            data.onDrag = false;

            $this.spritespin("animate", true);  // start animation

            // 被拖拽的 div css 进行还原
            //dragObj_isDrag = false;

            //console.info('up 了===================================================');

            ////console.info('内部的 mouseup');
            //dragObj_isDown = false;
            //$(dragObj_id).removeAttr("style");

        },
        mouseleave : function(e){
            //console.info('behaviors.drag.mouseleave');

            var $this = $(this), data = $this.data('spritespin');
            Spin.resetInput(data);
            data.onDrag = false;

            /*//console.info('内部的 mouseleave');
            dragObj_isDown = false;
            $(dragObj_id).removeAttr("style");*/
...
(function($, window, Spin){
    Spin.behaviors.hold = {
        name : "hold",
        mousedown  : function(e){
            //console.info('behaviors.hold.mousedown');
        ...
        mousemove  : function(e){
            //console.info('behaviors.hold.mousemove');
        ...
        mouseup    : function(e){
            //console.info('behaviors.hold.mouseup');
        ...
        mouseleave : function(e){
            //console.info('behaviors.hold.mouseleave');
        ...
        onFrame : function(e){
            //console.info('behaviors.hold.onFrame');
...
(function($, window, Spin){
    Spin.behaviors.swipe = {
        name : "swipe",
        mousedown  : function(e){
            //console.info('behaviors.swipe.mousedown');
        ...
        mousemove  : function(e){
            //console.info('behaviors.swipe.mousemove');
        ...
        mouseup    : function(e){
            //console.info('behaviors.swipe.mouseup');
        ...
        mouseleave : function(e){
            //console.info('behaviors.swipe.mouseleave');
...
(function($, window) {
    ...
    Module.drawSpritesheet = function(data){
        //console.info('Module.drawSpritesheet');
    ...
    Module.drawImages = function(data){
        //console.info('Module.drawImages');

        var img = data.images[data.frame];
        var whei = data.imgHeight;
        if (data.canvas){
            data.context.clearRect(0, 0, data.width, data.height);
            data.context.drawImage(img, 0, 0, whei*4/3, whei);
            return;
        }else{
            // 修改 img 的样式以及属性
            $(img).css({
                width: whei*4/3,
                height: whei,
                'margin-left': data.offset*4/3,
                'margin-top': data.offset,
                'opacity': data.opacity
            }).attr('usemap', '#' + data.frame);
        }
...
(function($, window) {
    ...
    Module.reload = function(data){
        ...
        for(i = 0; i < data.source.length; i+= 1){
            var img = $("<img src='" + data.source[i] + "' usemap='"+ i +"'/>");
    ...
    Module.resetInput = function(data){
        //console.info('Module.resetInput');
...
(function($, window) {
    ...
    Module.drawFirst = function(data){
        //console.info('Module.drawFirst 1');
